...
class HungarianTracker(Tracker):
    ...
    def iou_tracking(self, output_csv: str):
        ...
        for n_frame, img_file in tqdm(enumerate(self.img_file_list, start=1)):
            ...
            similarity_matrix_df = self.similarity_matrix(
                frame_data=frame_data, next_frame_data=next_frame_data
            )
            cost = 1 - similarity_matrix_df
            row_indices, col_indices = linear_sum_assignment(cost)
            assert cost.shape[0] == len(
                frame_data
            ), f"{cost.shape[0]} != {len(frame_data)}"
            assert cost.shape[1] == len(
                next_frame_data
            ), f"{cost.shape[1]} != {len(next_frame_data)}"
            print(cost)
